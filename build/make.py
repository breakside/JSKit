...
class Builder(object):
    ...
    def setup(self):
        ...
        self.info = PlistWrapper(os.path.join(self.sourceRootPath,'Info.plist'))
        ...
        if includedSourcePath:
            if includedSourcePath not in self.importedScripts:
                    ...
                    if isinstance(line,JSImport):
                        if line.delay:
                            ...
                            try:
                                ...
                                startedOutput = False
                            except IncludeNotFoundException as e:
                ...
                if self.debug:
                    ...
                    self.javascriptFile.write("""document.write('<script type="text/javascript" src="%s"></script>');""" % relativePath)
                    startedOutput = True
    ...
    def buildPlists(self):
        ...
        self.javascriptFile.write("var _JSApplicationResources = {};\n");
        mainUIFile = self.info['JSMainUIDefinitionFile']
        if mainUIFile:
            resources.append(mainUIFile)
        for resourcePath in resources:
            self.javascriptFile.write("_JSApplicationResources['%s'] = %s;\n" % (resourcePath, json.dumps(plistlib.readPlist(os.path.join(self.sourceRootPath,resourcePath)))))
    ...
    def buildIndex(self):
        ...
        title.appendChild(document.createTextNode(self.info['JSApplicationTitle'] or "Untitled Application"))
        ...
        html5(document).writehtml(self.indexFile)
...
class html5(object):
    ...
    RAW_ELEMENTS    = ('script','style','title')
        ...
        def _node(node):
            if node.nodeType == xml.dom.Node.ELEMENT_NODE:
                ...
                if node.tagName in self.VOID_ELEMENTS:
                    ...
                    if node.childNodes.length:
                        ...
                        for child in node.childNodes:
                            if child.nodeType not in (xml.dom.Node.TEXT_NODE, xml.dom.Node.ENTITY_NODE):
                                self.onlyTextChildren.append(False)
                                break
                    ...
                    elif node.tagName in self.RAW_ELEMENTS:
                        writer.write("></%s>" % node.tagName)
                    else:
                        writer.write("/>")
                    writer.write("\n")
    ...
    def tohtml(self):
        ...
        self.writehtml(writer)
        html = writer.getvalue()
        writer.close()
        return writer
