...
/* global Deflate, DeflateStream, JSLog, Adler32 */
'use strict';
...
var Zlib = {
    ...
    Error: function(msg){
        if (this === undefined){
            return new Zlib.Error(msg);
        }
        this.name = "Zlib.Error";
        this.message = msg;
    }

};

Zlib.Error.prototype = Object.create(Error.prototype);

var ZlibStream = function(input){
    if (this === undefined){
...
ZlibStream.prototype = {
    ...
    uncompress: function(){
        if (this.input.length < 6){
            throw new Zlib.Error("Zlib format is invalid, not enough bytes for header and checksum");
        }
        this.readHeader();
        ...
        if (this.method == Zlib.Method.Deflate){
            ...
            throw new Zlib.Error("Zlib method unknown: %d".sprintf(this.method));
        }
        this.checkOutput();
    },
    ...
    readHeader: function(){
        ...
        if (flags & 0x20){
            if (this.length - 4 < this.offset + 4){
                throw new Zlib.Error("Zlib format is invalid, not enough bytes for header with dictionary");
            }
        ...
        if (sum % 31){
            throw new Zlib.Error("Zlib header check failed: %#02x %#02x".sprintf(method_and_info, flags));
        }
    },
    ...
    checkOutput: function(){
        ...
        if (given[0] != calculated){
            throw new Zlib.Error("Zlib checksum failed: %d != %d".sprintf(given[0], calculated));
        }
    }
};