...
function HTMLAppBootstrapper(preflightID, preflightSrc, appSrc, body){
    if (this === undefined){
        ...
        if (HTMLAppBootstrapper.mainBootstrapper){
        ...
        window.addEventListener('error', function HTMLAppBootstrapper_handleWindowError(e){
            var src;
            for (src in bootstrapper.scriptConfigs){
                if (src.length <= e.filename.length){
                    if (e.filename.substr(e.filename.length - src.length) == src){
                        e.preventDefault();
                        bootstrapper.caughtErrors.push(e);
                        bootstrapper.scriptConfigs[src].compileError = e;
                        break;
                    }
                }
            }
...
HTMLAppBootstrapper.prototype = {
    ...
    loadPreflight: function(){
        ...
        var bootstrapper = this;
        this.include(this.preflightSrc, true, function(){
            bootstrapper.performChecks();
