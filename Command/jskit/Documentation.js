...
JSClass("Documentation", JSObject, {
    ...
    typescript: async function(){
        ...
        let frameworks = [];
        let typenames = [];
        let header = "// Auto-generated by `jskit doc --typescript`\n";
        header += "// To make updates, edit documentation and regenerate\n";
        await this.fileManager.createDirectoryAtURL(this.outputDirectoryURL);
        let visit = function(component){
            if (component.sourcePackage === "jskit"){
                return;
            }
            this.printer.setStatus("Scanning %s...".sprintf(component.name));
            if (component.kind === "framework"){
                for (let i = 0, l = component.children.length; i < l; ++i){
                    visit.call(this, component.children[i]);
                }
                let filename = "%s.d.ts".sprintf(component.name);
                let url = this.outputDirectoryURL.appendingPathComponent(filename);
                let typescript = component.typescriptDeclaration();
                if (typescript !== null){
                    typescript = header + "\n" + typescript;
                    let promise = this.fileManager.createFileAtURL(url, typescript.utf8());
                    frameworks.push([filename, promise]);
                    typenames.push(component.name);
                }
            }else{
                if (component.children){
                    for (let i = 0, l = component.children.length; i < l; ++i){
        ...
        visit.call(this, rootComponent);
        for (let [filename, promise] of frameworks){
            this.printer.setStatus("Writing %s...".sprintf(filename));
            await promise;
        }
        let typescript = header;
        for (let typename of typenames){
            typescript += '/// <reference types="./%s" />\n'.sprintf(typename);
        }
        let filename = "%s.d.ts".sprintf(rootComponent.name);
        let url = this.outputDirectoryURL.appendingPathComponent(filename);
        this.printer.setStatus("Writing %s...".sprintf(filename));
        await this.fileManager.createFileAtURL(url, typescript.utf8());


        this.printer.setStatus("Checking typescript...");
        let config = await this.loadPathAdjustedTSConfig();
        ...
        if (config.compilerOptions){
            if (config.compilerOptions.types){
                args.push("--types");
                args.push(config.compilerOptions.types.join(","));
            }
            if (config.compilerOptions.typeRoots){
                args.push("--typeRoots");
                args.push(config.compilerOptions.typeRoots.join(","));
            }
        }
        args.push(this.fileManager.pathForURL(url));
        let cwd = this.fileManager.pathForURL(this.workingDirectoryURL);
    ...
    loadPathAdjustedTSConfig: async function(){
        let url = this.workingDirectoryURL;
            ...
            if (config.compilerOptions){
                if (config.compilerOptions.typeRoots){
                    for (let i = 0, l = config.compilerOptions.typeRoots.length; i < l; ++i){
                        let typeRootURL = JSURL.initWithString(config.compilerOptions.typeRoots[i], url);
                        config.compilerOptions.typeRoots[i] = this.fileManager.pathForURL(typeRootURL);
                    }
                }
                if (config.compilerOptions.types){
                    for (let i = 0, l = config.compilerOptions.types.length; i < l; ++i){
                        if (config.compilerOptions.types[i].startsWith(".")){
                            let typeURL = JSURL.initWithString(config.compilerOptions.types[i], url);
                            config.compilerOptions.types[i] = this.fileManager.pathForURL(typeURL);
                        }
                    }
                }
            }
    ...
    loadSource: async function(url){
        ...
        component.sourcePackage = sourcePackage;
        return component;
    },
